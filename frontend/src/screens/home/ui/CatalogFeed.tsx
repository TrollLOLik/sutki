...
import { CompactAlert, EmptyState, ListCell, Skeleton } from '@ui';
...
export function CatalogFeed({ layout, headerCollapsed, loading, showingSimilar, guest, favoritesOnly, listings, favorites, onAuth, onToggleFavorite, onOpenListing, onBookListing, onEditListing, onPromoteListing }: CatalogFeedProps) {
  return (
    <main className={`listing-feed ${layout === 'grid' ? 'grid-layout' : 'list-layout'} ${headerCollapsed ? 'header-is-collapsed' : ''}`}>
      ...
      {loading ? <SkeletonFeed layout={layout} /> : listings.length ? listings.map((listing, index) => <ListingCard key={listing.id} listing={listing} layout={layout} mode="status" favorite={favorites.has(listing.id)} onToggleFavorite={() => onToggleFavorite(listing.id)} onOpen={() => onOpenListing(listing.id)} onBook={listing.isOwn ? undefined : () => onBookListing(listing.id)} onEdit={listing.isOwn ? () => onEditListing(listing.id) : undefined} onPromote={listing.isOwn ? () => onPromoteListing(listing.id) : undefined} style={{ '--listing-index': index } as CSSProperties} />) : <EmptyState icon={<Search size={34} />} title="Ничего не найдено" description="Измените запрос или сбросьте фильтры." />}
    </main>
  );
}
